...
if [ "$num_clusters" -gt 1 ]; then
	python $citup_directory/bin/clustering.py -i $results_location/Frequencies.txt -o $results_location/Clustered_mut.txt -c $num_clusters	-cf $results_location/cluster_membership.txt
fi

echo maximum number of nodes = $max_nodes
echo number of clusters = $num_clusters
for num_nodes in $(seq $min_nodes $num_clusters)
do 			
	echo number of nodes = $num_nodes
    if [ "$num_clusters" -gt 1 ]; then
		python $citup_directory/bin/clustering.py -i $results_location/Frequencies.txt -o $results_location/Clustered_mut${num_nodes}.txt -c $num_nodes -cf $results_location/cluster_membership${num_nodes}.ctr.txt
