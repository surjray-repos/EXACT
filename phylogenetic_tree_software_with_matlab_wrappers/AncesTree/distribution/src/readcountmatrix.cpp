...
ReadCountMatrix ReadCountMatrix::collapse(StlIntMatrix& toOrgColumns) const
{
  ...
  std::ofstream solution_cluster_file;
  solution_cluster_file.open( "solution_cluster_file.txt" );

  std::cerr << "\nInside the ReadCountMatrix::collapse function! \n";
  
  for (int i = 0; i < nrNewMutations; ++i)
  {
    snprintf(buf, 1024, "%d", i);
    std::cerr << "_rowLabel " << i << " " << buf << "\n";

    R._rowLabel[i] = buf;
    const StlIntVector& M = toOrgColumns[i];
    for (StlIntVectorIt it = M.begin(); it != M.end(); ++it)
    {
      std::cerr << " value of i = " << i << " value of it row number = " << *it << "\n";
      solution_cluster_file << " value of i = " << i << " value of it row number = " << *it << std::endl;
      for (int sample = 0; sample < _n; ++sample)
      {
        std::cerr << " value of sample = " << sample << "\n";
        R._C[i][sample] += _C[*it][sample];
        R._D[i][sample] += _D[*it][sample];
        std::cerr << _D[*it][sample] << " " << _C[*it][sample] << " ";
      }
      std::cerr << "\n";
    }
  }
...
{
  std::cerr << "\nInside the ReadCountMatrix::computePointEstimates function! \n";
  F = RealMatrix(_n, _m);
  for (int i = 0; i < _n; ++i)
  {
