...
//      printVariables(solIdx, std::cout);
  ...
  for (int i = 0; i < 5*18; i++){
		printf(" %f ", _cplex.getValue(_f[ (i % 5) ][(i/5)]));
		fflush(stdout);
  }
...
void IntMaxIlpSolver::printVariables(int solIdx, std::ostream& out) const
{
  ...
  ofstream solution_x_file;
  ofstream solution_f_file;
  solution_x_file.open( "solution_x_file.txt" );
  solution_f_file.open( "solution_f_file.txt" );

  out << "Solution " << solIdx << std::endl;
  out << "Obj value: " << _cplex.getObjValue(solIdx) << std::endl;
  solution_x_file << "Solution " << solIdx << endl;
  solution_x_file << "Obj value: " << _cplex.getObjValue(solIdx) << endl;
  ...
  for (int i = 0; i < _arcCount + _nodeCount; ++i)
  {
    ...
    if (val)
      out << _x[i].getName() << " = " << val << std::endl;
  	  solution_x_file << _x[i].getName() << " = " << val << endl;
  }
  
  solution_f_file << "Solution " << solIdx << endl;
  solution_f_file << "Obj value: " << _cplex.getObjValue(solIdx) << endl;
  ...
  for (int i = 0; i < m; ++i)
  {
    for (int j = 0; j < n; ++j)
    {
      ...
      solution_f_file << _f[i][j].getName() << " = " << _cplex.getValue(_f[i][j], solIdx) << endl;
    }
  }
}
...
printVariables(solIdx, std::cout);
printf("===Solution %d=====\n", solIdx);
ofstream solution_number_file;
solution_number_file.open( "solution_number_file.txt" );
solution_number_file << "Solution no." << solIdx << endl;

for (NodeIt v(G); v != lemon::INVALID; ++v)
{
int idx = _nodeToRootArcIndex[v];
	printf("%d: %.5f\n", idx, _cplex.getValue(_x[idx], solIdx));
	solution_number_file << idx << ":" << fixed << setprecision(5) << _cplex.getValue(_x[idx], solIdx) << endl;
}
for (ArcIt a(G); a != lemon::INVALID; ++a)
{
    int idx = _arcToIndex[a];
    printf("%d: %.5f\n", idx, _cplex.getValue(_x[idx], solIdx));
    solution_number_file << idx << ":" << fixed << setprecision(5) << _cplex.getValue(_x[idx], solIdx) << endl;
}
printf("==================\n");
solution_number_file << "==================\n";
  ...
  for (SubDigraph::NodeIt v_j(subG); v_j != lemon::INVALID; ++v_j)
  {
    ...
    for (int i = 0; i < m; ++i)
    {
      ...
      printf("===processSolution***_cplex.getValue(_f[i][j]) = %f***==\n", f_ij);
      ...
      if (g_tol.less(f_ij, sum)){
          printf("**&&**ERROR**&&** Sum condition violated\n");
      }
    }
  }
...
void IntMaxIlpSolver::initVariables()
{
  ...
  ofstream ILP_construction;
  ILP_construction.open( "ILP_construction.txt" );
  ILP_construction << "Constructing ILP..." << endl;
...
void IntMaxIlpSolver::initConstraints()
{
  ...
  //File to store the _f values, right after the ILP stage
  ofstream solution_f_hat_file;
  solution_f_hat_file.open( "solution_f_hat_file.txt" );
...
//        int j = _nodeToIndex[v_j];
  ...
  solution_f_hat_file << m << endl;
  solution_f_hat_file << n << endl;
  // VAF deviation
  cout << "_f[i][j] values are similar to the CI matrix!" << endl;
  for (int i = 0; i < m; ++i)
  {
    for (int j = 0; j < n; ++j)
    {
      ...
      for (StlIntVectorIt it = M.begin(); it != M.end(); ++it)
      {
        ...
        cout << "_F_point_unclustered : " << f << endl;
        solution_f_hat_file << f << endl;
		    cout << "_f[i][j] = " << _f[i][j] << endl;
