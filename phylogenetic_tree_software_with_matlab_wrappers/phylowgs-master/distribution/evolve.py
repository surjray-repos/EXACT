...
def start_new_run(state_manager, backup_manager, safe_to_exit, run_succeeded, config, ssm_file, cnv_file, params_file, top_k_trees_file, clonal_freqs_file, burnin_samples, num_samples, mh_itr, mh_std, write_state_every, write_backups_every, rand_seed, tmp_dir):
	...
	print('Value of datum in evolve.py = ')
	for datum in codes:
	...
	print('Value of NTPS in evolve.py = ')
	print(NTPS)
	print('Value of sample codes[0] object in evolve.py = ')
	print(codes[0].a)
	for attr in dir(codes):
		print("codes.%s = %r" % (attr, getattr(codes, attr)))
...
def do_mcmc(state_manager, backup_manager, safe_to_exit, run_succeeded, config, state, tree_writer, codes, n_ssms, n_cnvs, NTPS, tmp_dir_parent):
	...
	print( codes ) 
	print( '\n' )
	print('Value of sample codes[0] object in evolve.py do_mcmc = ')
	print(codes[0].a)
	...
	for iteration in range(start_iter, state['num_samples']):
		...
		if iteration >= 0:
			...
			if True or mod(iteration, 10) == 0:
				...
				print('weights, nodes = %s %s', weights, nodes)
...
def parse_args():
	...
	parser.add_argument( '-nS', '--new-state', dest='new_state_switch', default=0, type=int, help='To create a new state and run-environment!' )
...
def run(safe_to_exit, run_succeeded, config):
	...
	if state_manager.state_exists() and args.new_state_switch != 1:
